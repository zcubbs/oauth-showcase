...
func StartServer() {
	...
	for _, client := range configs.Cfg.Clients {
		...
		if err != nil {
			log.Printf("Error setting client [%s]: %v", client.ID, err)
		}
	}
	...
	http.HandleFunc("/login", serverLoginHandler)
...
func serverLoginHandler(w http.ResponseWriter, r *http.Request) {
	if debug {
